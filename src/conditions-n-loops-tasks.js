...
function isContainNumber(num, digit) {
  if (getIndexOf(`${num}`, `${digit}`) >= 0) return true;
  return false;
}
...
function getSpiralMatrix(size) {
  ...
  for (let k = 1; k <= size * size; k += 1) {
    ...
    if (tst < 0 || tst === size || tst === -1) {
      tst = dx;
      dx = -dy;
      dy = tst;
    } else if (arr[j + dy][i + dx] > 0) {
      tst = dx;
      dx = -dy;
      dy = tst;
    }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1)
    for (let j = 0; j < n; j += 1) result[j][i] = matrix[n - i - 1][j];
  const m = matrix;
  for (let i = 0; i < n; i += 1)
    for (let j = 0; j < n; j += 1) m[i][j] = result[i][j];
  return matrix;
}
...
function getNearestBigger(number) {
  ...
  let max = 10;
  let nmax = -1;
  for (let i = 0; i < num.length; i += 1) numArr[i] = parseInt(num[i], 10);
  ...
  for (let i = n + 1; i < numArr.length; i += 1)
    if (numArr[i] > numArr[n] && numArr[i] < max) {
      max = numArr[i];
      nmax = i;
    }
  let tmp = numArr[n];
  ...
  let result = '';
  for (let i = 0; i < numArr.length; i += 1) result += numArr[i];
